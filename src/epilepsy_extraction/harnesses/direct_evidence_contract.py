...
def run_direct_evidence_contract(
    ...
    *,
    ...
    prompt = load_prompt("direct_evidence_contract")
    ...
    return RunRecord(
        run_id=run_id,
        harness="direct_evidence_contract",
        ...
        rows=rows,
        metrics=_aggregate_evidence_support(evidence_support_rows),
...
def _call_provider(
    ...
    return provider.complete(
        ProviderRequest(
            ...
            metadata={
                "prompt_id": "direct_evidence_contract",
...
def _payload_from_response(
    ...
    record: GoldRecord,
) -> tuple[ExtractionPayload, list[tuple[str, bool]], dict[str, Any]]:
    warnings: list[str] = []
    if not response.ok:
        warnings.append("provider_error")
        return _invalid_payload(record, warnings), _failed_validity(), _empty_support()
    ...
    try:
        ...
        return _invalid_payload(record, warnings, raw=decoded), _failed_validity(), _empty_support()
    ...
    support_stats = _check_evidence_support(final, record.letter)
    unsupported = support_stats.get("unsupported_citations", 0)
    payload_warnings = [str(w) for w in final.warnings] + warnings
    if unsupported:
        payload_warnings.append(f"unsupported_citations={unsupported}")

    return (
        ExtractionPayload(
            pipeline_id="direct_evidence_contract",
            final=final,
...
def _check_evidence_support(final: FinalExtraction, letter: str) -> dict[str, Any]:
    total = len(final.citations)
    supported = sum(1 for c in final.citations if c.get("quote") and c["quote"] in letter)
    unsupported = total - supported
    return {
        "total_citations": total,
        "supported_citations": supported,
        "unsupported_citations": unsupported,
        "evidence_supported_rate": supported / total if total else 1.0,
    }


def _aggregate_evidence_support(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if not rows:
        return {}
    total_cites = sum(r.get("total_citations", 0) for r in rows)
    supported = sum(r.get("supported_citations", 0) for r in rows)
    return {
        "evidence_support": {
            "total_citations": total_cites,
            "supported_citations": supported,
            "unsupported_citations": total_cites - supported,
            "evidence_supported_rate": supported / total_cites if total_cites else 1.0,
        }
    }
...
def _failed_validity() -> list[tuple[str, bool]]:
    return [("final_output", False)] + [(f.value, False) for f in CORE_FIELD_FAMILIES]
